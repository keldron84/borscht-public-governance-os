...
def execute_action(action_type: str, params: Dict[str, Any]) -> ExecutionResult:
    ...
    if action_type in ("analysis", "draft", "summary", "internal_note", "markdown_export", "content_publish"):
        ...
        return ExecutionResult(
            ...
            detail="Generated markdown artifact ({0} chars).".format(len(body)),
        )
    ...
    if action_type == "json_export":
        ...
        return ExecutionResult(
            ...
            content=content, ext="json", detail="Exported JSON payload.",
        )
    ...
    if action_type in ("http_get",):
        ...
        try:
            ...
            return ExecutionResult(
                ...
                content=data, ext="txt", detail="Fetched {0} (truncated 5k).".format(url),
            )
    ...
    if action_type in ("webhook_post", "outbound_message", "spend_increase", "campaign_launch", "data_mutation", "data_write", "budget_change", "external_publish"):
        ...
        return ExecutionResult(
            ok=True, kind="json", title="Action: {0}".format(action_type),
            content=content, ext="json",
            detail="Executed {0} via simulated connector.".format(action_type),
            raw={"simulated": True},
        )
    ...
    if action_type == "file_write":
        ...
        return ExecutionResult(
            ...
            content=body, ext="txt", detail="Wrote {0}.".format(rel),
        )

    if action_type == "manual_handoff":
        note = params.get("note", "Manual task handed to a human.")
        return ExecutionResult(
            ok=True, kind="markdown", title="Manual handoff",
            content="# Manual handoff\n\n{0}\n".format(note), ext="md",
            detail="Handed off to human operator.",
        )
...
def _render_default_markdown(params: Dict[str, Any]) -> str:
    lines = ["# {0}".format(params.get("title", "Result")), ""]
    for k, v in params.items():
        if k in ("title", "body", "text"):
            continue
        lines.append("- **{0}:** {1}".format(k, v))
    lines.append("")
    lines.append("_Generated by Borscht Public Edition._")
    return "\n".join(lines)
